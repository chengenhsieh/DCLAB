module Top (
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            ...
            i2c_start_r <= 1'b0;       // start 保持 0，在復位後延遲一個 clk 再拉高
            rec_start_r <= 1'b0;
            dsp_start_r <= 1'b0;

            i2c_poweron_delay <= 1'b0; // delay flag 清 0
        end
        else begin
            state_r <= state_w;
            ...
            // one-cycle delay after reset: first clock sets poweron_delay,
            // next clock (when poweron_delay==1) we assert i2c_start_r (unless already done)
